...
HeroManager manager  =  new HeroManager();//生成管理对象 manager.
int id  = Integer.parseInt(request.getParameter("id"));//从get的url获得id,以得到hero
String isChange  =
request.getParameter("is_change");//从get的url获得is_change的值,以判断是要查看还是编辑一个英雄

String directPage  = "";//将要转向的页面。
Hero hero  = manager.findOne(id);//使用manager对象和刚才传递过来的  id，获取英雄
request.setAttribute("hero", hero);//将获取到的英雄放到 request里面去。
//如果url的is_change对象不为空，说明是需要跳转到编辑页面   .
//否则是英雄的显示页面  .
if(isChange  !=  null){
...
String idStr  = request.getParameter("id");//从表单获得英雄 id
HeroManager manager  =  new HeroManager();//使用manager对象和刚才传递过来的  id，获取英雄
Hero hero  =  new Hero();//声明一个新的英雄对象
